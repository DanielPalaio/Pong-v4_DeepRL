...
def DeepQNetwork(lr, num_actions, input_dims, fc1):
    q_net = Sequential()
    q_net.add(Conv2D(filters=32, kernel_size=8, strides=4, activation='relu', input_shape=(*input_dims,),
                     data_format='channels_first'))
    q_net.add(Conv2D(filters=64, kernel_size=4, strides=2, activation='relu', data_format='channels_first'))
    q_net.add(Conv2D(filters=64, kernel_size=3, strides=1, activation='relu', data_format='channels_first'))
    q_net.add(Flatten())
...
class Agent:
    def __init__(self, lr, discount_factor, num_actions, epsilon, batch_size, input_dims):
        ...
        self.step_counter = 0
        self.buffer = ReplayBuffer(25000, input_dims)
        self.q_net = DeepQNetwork(lr, num_actions, input_dims, 512)
        self.q_target_net = DeepQNetwork(lr, num_actions, input_dims, 512)
    ...
    def train(self):
        ...
        for idx in range(done_batch.shape[0]):
            target_q_val = reward_batch[idx] + self.discount_factor*q_max_next[idx]*(1 - done_batch[idx])
    ...
    def train_model(self, env, num_episodes, graph):
        ...
        for i in range(num_episodes):
            ...
            print("Episode {0}/{1}, Score: {2} ({3}), AVG Score: {4}, Steps: {5}".format(i, num_episodes, score,
